...
# D²-FL Core Library
# =============================================================================
# 联邦学习核心模块：数据加载、模型定义、训练更新、原型聚合、分布原型、解耦
#
# 子包：
#   dist_proto/  — 分布原型（Gaussian head / Bayesian fusion / disentanglement）
#   models/      — 骨干网络（ResNet50 / D2FLResNet）
#   dp/          — 隐私说明（DP 代码已移除，隐式隐私见 README）
#
# 顶层模块：
#   options.py   — 命令行参数解析
#   utils.py     — 数据集加载、原型聚合、实验信息打印
#   update.py    — 本地训练循环、测试推理（6 种算法的客户端 + 评估逻辑）
...
__all__ = [
    ...
    'test_inference_new_het_lt', 'test_inference_new_het_lt_D2FL',
    'eval_clients_multilabel',

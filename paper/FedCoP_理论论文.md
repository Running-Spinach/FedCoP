...
这不止是不便,而是一个决定了解法的事实。全局 $C\times C$ 共现结构*只能*通过对各客户端充分统计量进行联邦聚合来恢复,且需要那些类支持联合覆盖全部 $C$ 类的客户端。因此,共现不是碰巧在联邦系统里运行的多标签技巧,而是一个**结构上必须联邦**的量——对任何参与者不可见,只对作为整体的联邦可见。这是 FedCoP 的核心洞察,也是我们把共现矩阵当作一等联邦对象、而非事后修正的原因。
...
3. **理论。** 给出解码器 regret 界,证明 mean-field 解码器不劣于独立 sigmoid,当 $\hat R$ 追踪到残差标签相关时严格更优;并给出一个联邦估计定理(无偏性、matrix-Bernstein 集中、单客户端不可恢复性),证明该结构必须是联邦的。
...
**【中文】** **FedCoP 的不同之处。** 上述所有方法在原型几何与解码两方面都把 $C$ 个类别独立处理。FedCoP 是首个联邦地估计*跨类*共现结构、并在训练(几何)与推理(解码)两端都加以利用的方法。在集中式学习中已存在多标签标签相关方法(分类器链、标签嵌入),但它们在非独立同分布的类别划分下无法恢复该结构——联邦估计正是我们的贡献。
